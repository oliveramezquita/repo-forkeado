...
<template>
    <form @submit.prevent="submit">
        ...
        <div class="mb-3">
            ...
            <input v-model="form.name" type="text" class="input-text" id="name" placeholder="Ingresa el nombre" required autofocus minlength="4">
        </div>

        <div class="mb-3">
            <label for="max_failed_subjects_to_pass" class="form-label">Máximo de asignaturas suspensas</label>
            <input v-model="form.max_failed_subjects_to_pass" type="numeric" class="input-text" id="max_failed_subjects_to_pass" required min="0" max="99">
        </div>
        ...
        <div class="d-flex flex-column mt-4">
            ...
            <DangerButton v-if="props.edit === 'true' && props.obj.is_active == '1' && permissions.includes('degree.destroy')" 
                        class="btn desactive-button w-100" 
                        :class="{ 'opacity-25': form.destroy }" 
                        :disabled="form.destroy"
                        @click="destroy">
                Desactivar
            </DangerButton>
            
            <SecondaryButton v-if="props.edit === 'true' && props.obj.is_active == '0' && permissions.includes('degree.activate')" 
                            class="btn submit-button w-100" 
                            :class="{ 'opacity-25': form.activate }" 
                            :disabled="form.activate" 
                            @click="activate">
                Reactivar
            </SecondaryButton>
        </div>
...
.success-feedback {
    background-color: #229e26; /* Por ejemplo, verde. Ajusta según tus necesidades. */
    transition: background-color 0.3s ease;
}

.success-feedback:hover {
    background-color: #19af21; /* Un tono de verde un poco más oscuro en el hover. Ajusta según tus necesidades. */
}
...
.white-icon {
    ...
    color: white; /* Para asegurarte de que el ícono sea blanco */
}
